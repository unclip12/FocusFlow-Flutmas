...
calculateNextRevisionDate(lastStudied, revisionIndex, revisionSettings) → DateTime?
  ...
  - Else → lastStudied + schedule[revisionIndex] hours
```
...
- Two modes: MENTOR (general AI tutor) / BUDDY (uses Info Files as context)
...
- View Today's Plan link in chat
- Persistent conversation history
- AI Settings accessible from here

### 14. My AI Memory
- Display name edit
- Exam target (e.g. FMGE)
- Exam date picker
- Learning style freetext
- Typical delay patterns
- Preferred AI tone (strict/encouraging/balanced)
- Backlog items list (pending tasks from previous days)
- Notes field
- Memory permission toggles (can AI read KB, time logs, info files)
...
- Notification mode: normal / strict
- Notification types: block timers, breaks, mentor nudges, daily summary
...
- Top header bar on every screen:
  - Left: Current screen name (TAPPABLE)
  - Right: Streak indicator (fire + number)
- Tap screen name → opens nav overlay with ALL visible screens
- Nav overlay: spring/bounce animation (overshoot), anchored top-left
- Select any screen → navigates + closes overlay
- Currently active screen highlighted in overlay
- Respects `menuConfiguration` (hidden screens not shown)
...
// revisionSettings.mode → get schedule list
// If revisionIndex >= schedule.length → null (mastered)
...
// Must match web app Firebase export format for cross-restore

// importBackup(jsonString) → parse all keys → write to SQLite
// Show progress during import
```

### fa_logger_service.dart
```dart
// performFullIntegrityCheck(kb, revisionSettings)
//   → validates all SRS dates are consistent
//   → returns { updated: bool, data: KnowledgeBaseEntry[] }

// recalculateEntryStats(entry, revisionSettings)
//   → recalcs nextRevisionAt from logs
```
...
| 8 | screens/todays_plan/add_block_modal.dart, add_break_modal.dart, pause_reason_modal.dart, manual_plan_modal.dart, reflection_modal.dart | Today's Plan modals |
...
| 26 | Streak logic, Due Now, activity graph data, PROGRESS.md update | Final features |
